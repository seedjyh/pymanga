...
class AntbywSpider(scrapy.Spider):
    ...
    def parse_comic_page(self, response):
        ...
        comic_title = response.xpath("//h3[contains(@class, 'uk-heading-line mt10 m10 mbn')]/text()").extract_first()
        ...
        yield l.load_item()
        # scrape volume url
        for volume_item in response.xpath("//div[@class='muludiv']/a[contains(@class, 'zj-container')]"):
            volume_url = volume_item.xpath("@href").get()
            real_volume_url = urllib.parse.urljoin(response.url, volume_url)
            yield Request(url=real_volume_url, callback=self.parse_volume_page,
                          meta={'comic_path': comic_path, 'comic_title': comic_title, })

    def parse_volume_page(self, response):
        """parse a volume page"""
        root_path = settings.DOWNLOAD_STORE
        comic_title = response.meta.get("comic_title",
                                        response.xpath('//script/text()').re_first('g_comic_name = "(.+)"'))
        comic_path = response.meta.get("comic_path", os.path.join(root_path, comic_title))
        volume_title = response.xpath("//h3[@class='uk-heading-line uk-text-center']/span/text()").extract_first()
        print("=========================================================================")
        print("comic_path=", comic_path, "volume_title=", volume_title)
        print("=========================================================================")
        ...
        pic_url_list = response.xpath(
            "//div[@class='uk-zjimg uk-text-center uk-padding uk-margin-top uk-margin-bottom uk-inline']/div/img/@data-src").extract()
